...
nonisolated enum GeminiSchemas {
    ...
    static func receiptResponseSchema(categoryNames: [String]) -> JSONValue {
        ...
        let lineItem: JSONValue = [
            ...
            "properties": [
                ...
                "canonicalName": ["type": "string", "description": "Clean human-readable product name per the canonicalization rules in the prompt."],
            ...
            "required": ["rawName", "canonicalName", "quantity", "unit", "unitPrice", "lineTotal", "suggestedCategory", "uncertain", "uncertaintyReason", "isDiscountOrDeposit"],
        ]
    ...
    static func receiptPrompt(
        ...
        return """
        You are a precise receipt-parsing engine for a Finnish household finance app. Analyze the receipt image and return JSON matching the provided schema. Follow these rules exactly.

        LANGUAGE AND NUMBERS
        - Receipts are in Finnish, sometimes bilingual Finnish/Swedish. Vocabulary: YHTEENSÄ/SUMMA = total, KÄTEINEN = cash, KORTTI/PANKKIKORTTI/LUOTTOKORTTI = card, ALV/MOMS = VAT, PANTTI = bottle deposit, PANTIN PALAUTUS = deposit return, ALENNUS/TARJOUS/ETU = discount, PYÖRISTYS = cash rounding, VÄLISUMMA = subtotal.
        - Finnish receipts use comma as decimal separator: "2,99" means 2.99. ALWAYS convert to dot-separated strings ("2.99"). Money strings: exactly 2 decimals, no currency symbol, '-' prefix for negatives.
        - Dates may be DD.MM.YYYY or DD.MM.YY. Convert to YYYY-MM-DD.
        ...
        - PANTTI lines (deposits, typically 0,10/0,15/0,40 EUR) are separate line items: isDiscountOrDeposit=true, canonicalName "Pantti", positive lineTotal. PANTIN PALAUTUS likewise with negative lineTotal.
        - Loyalty and campaign discounts (PLUSSA-ETU, S-Etukortti, Bonus, ALENNUS, TARJOUS, percentage discounts) are separate line items: isDiscountOrDeposit=true, negative lineTotal, canonicalName describing the discount (e.g. "Plussa-etu"). Use the category of the discounted product if obvious, otherwise the most general category in the list.
        - A PYÖRISTYS line (cash rounding, max ±0,04) is a line item with isDiscountOrDeposit=true, canonicalName "Pyöristys".
        ...
        - Style: clean, concise, human-readable, capitalized like a normal product name. Finnish or English, whichever is most natural; prefer the form already used in the KNOWN PRODUCTS list below.
    ...
    static func productIDResponseSchema() -> JSONValue {
        [
            "type": "object",
            "properties": [
                "productName": ["type": "string", "description": "Clean, concise product name per the canonical naming rules."],
                "brand": ["type": .array(["string", "null"])],
                "size": ["type": .array(["string", "null"]), "description": "Package size as printed, e.g. '1 l', '400 g'."],
                "confidence": ["type": "string", "enum": ["high", "medium", "low"]],
            ],
            "required": ["productName", "brand", "size", "confidence"],
        ]
    }

    static func productIDPrompt(knownProducts: [String]) -> String {
        let productsBlock = knownProducts.isEmpty ? "(none yet)" : knownProducts.joined(separator: "\n")
        return """
        Identify the grocery/household product shown in this photo of its packaging. Return JSON matching the schema.
        - productName: clean, concise, human-readable (Finnish or English, whichever is natural). Keep the brand when it distinguishes the product, and the size/variant when visible (e.g. "Arla laktoositon maito 1L").
